...
const authMiddleware = async (req, res, next) => {
    ...
    if (!accessTokenValidate) {
        ...
        if (!refreshTokenValidate) { //access 만료, refresh만료
            return next()

            // return true
        }

        //6. 새로운 accessToken 생성 //refresh만 있을 때
        // const user = await User.findOne({ where: { loginId } });
        // const { loginId } = payload;
        const newAccessToken = jwt.sign(payload.payload, JWT_SECRET_KEY, { expiresIn: '10s' });
