...
const { User, Hospital, Doctor, sequelize, WorkingTime } = require('../models');
...
class WorkingtimeService {
    ...
    findWorkingDate = async (year, month, date, week) => {
        ...
        const workingTimeKeys = Object.keys(reservaionObj);

        // console.log(workingTimeKeys);

        for (let index = 0; index < workingTimeKeys.length; index++) {
            const element = workingTimeKeys[index];
            if (reservaionObj[element] === '') {
                workingTimeDate.push(element);
            }
        }

        // console.log(workingTimeDate);
