class CreateError {
    ...
    UserNotLogined = () => {
        const createdError = Error('로그인이 필요합니다.');
        createdError.name = 'UserNotLogined';
        createdError.status = 401;
        return createdError;
    };

    roleNotAllow = () => {
        const createdError = Error('관리자 승인이 필요합니다.');
        createdError.name = 'roleNotAllow';
        createdError.status = 401;
        return createdError;
    };

    DoctorNotFound = () => {
        const createdError = Error('해당 의사가 존재하지 않습니다.');
        createdError.name = 'DoctorNotFound';
        createdError.status = 400;
        return createdError;
    };

    hospitalIsExisted = () => {
        const createdError = Error('해당 병원이 이미 존재합니다.');
        createdError.name = 'hospitalIsExisted';
        createdError.status = 400;
        return createdError;
    };

    notSelectRelationShip = () => {
        const createdError = Error('환자와의 관계를 선택해주세요.');
        createdError.name = 'notSelectRelationShip';
        createdError.status = 412;
        return createdError;
    };

    // TokenNotFound = () => {
    //     const createdError = Error('토큰이 유효하지 않습니다.');
    //     createdError.name = 'TokenNotFound';
    //     createdError.status = 401;
    //     return createdError;
    // };
}
